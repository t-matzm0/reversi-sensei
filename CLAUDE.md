...
- **Issue #20 のクローズ準備**
  ...
  - PR #29 マージ後の全体見直しを実施
  - 6概念（定石・偶数理論・中割り・ウイング・種石・ストーナー）すべて実装済みであることを確認
  - Issue #20 本文のチェックボックスを更新し、完了報告コメントを投稿（クローズはオーナー判断）

- **リポジトリのクリーンアップ**

  - stale worktree（.claude/worktrees/amazing-boyd）を削除（jest の haste collision とテスト二重実行を解消）
  - マージ済みローカルブランチ（feature/20-strategy-concepts、claude/amazing-boyd）を削除

- **Issue #13: チュートリアルに戦略概念の章を追加**

  - ツールチップに戦略名が表示されても学ぶ場所がなかったギャップを解消
  - 導入＋5概念（中割り・ウイング・偶数理論・種石・ストーナー）の6ステップを追加（計19ステップ）
  - 変更ファイル:
    - `src/lib/strategyExamples.ts`: 新規作成。各概念の例示盤面を定義
    - `src/components/Tutorial.tsx`: 戦略概念の章（6ステップ）を追加
    - `src/__tests__/lib/strategyExamples.test.ts`: 新規作成（8テスト）。**例示盤面が実際の検出ロジック（strategyDetection.ts）を発火させることを保証** — チュートリアルの説明とゲーム中のツールチップ表示の一貫性をテストで担保する設計
    - `src/__tests__/components/Tutorial.test.tsx`: 新規作成（3テスト）。全ステップ遷移と例示盤面の石数一致を検証

- **動作確認**
  - Lint: ✅ エラー/警告なし
  - TypeScript: ✅ 型エラーなし（tsc exit 0 を明示確認）
  - Test: ✅ 全111テスト合格（11スイート）
  - Build: ✅ production ビルド成功
  - dev サーバー（port 3001）でバンドルに新コンテンツが含まれることを確認

【次回への申し送り】

- Issue #13 のチュートリアル拡充 PR のレビュー（オーナー）
- Issue #20 のクローズ判断（オーナー）
- 残 Issue の優先順位案: #6（アニメーション）→ #7（棋譜記録）→ #12（UI/UX）→ #11（統計）→ #10（AI強化）→ #9（オンライン対戦）
